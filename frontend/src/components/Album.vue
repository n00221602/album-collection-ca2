...
<template>
  <RouterLink :to="`/albums/${props.album.id}`">
    <Card class="relative p-0 border-4 border-black">
      <img v-if="albumImage" :src="albumImage" :alt="props.album.title" class="p-0 rounded-lg" />
      <div
        class="absolute inset-0 bg-black opacity-0 hover:opacity-80 transition-opacity rounded-lg border-2 border-stone-900 flex items-center justify-center">
